...
public extension Data {
    ...
    /// Decode this data into a value, optionally using a specific decoder.
    /// If no explicit encoder is passed, then the data is decoded as JSON.
    /// If can't decode, will return nil
    func decodeOptional<T: Decodable>(as type: T.Type = T.self,
                                      using decoder: AnyDecoder = JSONDecoder()) -> T? {
        do {
            return try decoded()
        } catch {
            return nil
...
public extension Decoder {
    ...
    /// Decode a nested value using the format key1.key2.key3
    /// Throws an error if keys array is empty
    func decodeNested<T: Decodable>(_ keys: String, as type: T.Type = T.self) throws -> T {
        return try decode(keys.split(separator: ".").map { AnyCodingKey(String($0)) })
    }
